...
import pprint, binascii, uuid, traceback
...
def run_load(cfg):
    ...
    uplink = uplink_stream(stream_id=cfg['uplink_stream_id'],
    ...
    tunnel = tunnel_stream(stream_id=cfg['tunnel_stream_id'],
                           ...
                           trex_uplink_mac=cfg['trex_uplink_mac'],
                           flow_rate=cfg['flow_rate'])
    flowstate = flowstate_stream(gateway_count=cfg['gateway_count'],
                                 trex_underlay_mac=cfg['trex_underlay_mac'],
                                 agent_underlay_mac=cfg['agent_underlay_mac'],
                                 agent_underlay_ip=cfg['agent_underlay_ip'],
                                 agent_uplink_port_uuid=cfg['agent_uplink_port_uuid'],
                                 protobuf_flowstate=cfg['protobuf_flowstate'],
                                 flow_rate=cfg['flow_rate'])

    c.reset([cfg['tunnel_port_index'], cfg['uplink_port_index']])
    c.add_streams([uplink], ports=[cfg['uplink_port_index']])
    c.add_streams([tunnel, flowstate],
                  ports=[cfg['tunnel_port_index']])
    c.clear_stats()

    c.start(ports=[cfg['tunnel_port_index'], cfg['uplink_port_index']], 
    ...
    print "Running workload (%d fps for %d seconds)" % (cfg['flow_rate'],
                                                        cfg['duration'])
...
parser.add_argument('--flowrate', dest='flowrate', default=1000, type=int)
