...
[NOTE]
=====================================================================
Breaking change: service mode has 3 state now instead of only off/on +

1. *On* - all the packets are forward to rx 

2. *Off* - default. before v2.66 non TCP UDP were forward to rx in software mode after v2.66 (including) only in filter mode those packets are forward for more flexibility  +

3. *Filter* - in this case specific packets are forward to rx for example BGP packets for BIRD. It is relevant only for software mode `--software`` +
using this mode you would be able to run TCP/UDP traffic in high rate while keeping the routing protocols function 
=====================================================================
