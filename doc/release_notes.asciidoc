...
the client can consume only the information it really needs, when it needs it, so statistics for more flows can
be supported. **Notice** that in some future release, pgid info will not be supported using the old "get_stats" any more.
Example for using the new API can be found in "automation/trex_control_plane/stl/examples/stl_flow_latency_stats.py".
