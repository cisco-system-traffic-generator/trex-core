...
We mentioned before that in ASTF there are no drops, and the drops are replaced by errors. Errors however, don't have a percentage and once an error happens, it disqualifies that transmission (run) from being valid. At the time this document is written, there is no way to tolerate errors.
...
Same as in STL, in ASTF we can use plugins to decide what to do pre and post transmission. The only difference is that in ASTF since we don't transmit at maximum capacity, the API doesn't receive that parameter.
...
[source,bash]
----
[bash]>./ndr --astf -v --plugin-file automation/trex_control_plane/interactive/trex/examples/astf/astf_error_logging_plugin.py --high-mult 60 --low-mult 40
----

The plugin not only logs the errors and the multipliers but in case it finds out some error that is not allowed happened, it immediately stops the run. For example, in the default profile this won't happen but if we run some other profile, we will get the following results:
