...
* *RPC Representation Protocol* - The format in which remote procedures are carried
...
=== RPC Representation Protocol

The RPC representation protocol is JSON RPC v2.0.
...
* *Description* - Sync with server about API classes. This allows the server and the client
		  ...
		  The return values are used for further communication with the server.
...
|=================
| Field           | Type    | Description
| action_count    | uint32_t  | In case it is bigger than zero and next stream is not -1 (set) the number of goto will be limited to this number. Maximum value is 4G. default is zero. Zero means - not limit.
| core_id         | int     | Pins the stream to core_id.
...
| start_paused    | boolean | Stream will not be transmitted until un-paused.
| vm              | object  | array of objects of type xref:vm_obj['vm']
| flow_stats      | object  | Per stream statistic object. See: STLFlowStats.
...
The variable name.port and name.ip could be written to any offset in the packet (usually to src_ip and src_port as client) 
...
The use cases of this instruction is to write to a bit field (vlan/mpls)
...
* *override* - up to 10 bytes at the end of the packet will be overridden by the meta data required
...
This request-reply sequence demonstrate a method in which rx_stats are disabled.
...
* *API Class* - 'core'
* *Valid States* - 'unowned', 'owned', 'active'
* *Description* - fetch all the associated streams for a port
* *Parameters*
** *handler* ['string'] - unique connection handler
...
As an optional field, the profile ID allows TRex server to run multiple profiles dynamically in ASTF interactive mode.
The profile ID is used as the identifier of profile.

The string type 'profile_id' anchor:astf_profile_id[]

.String type 'astf_profile_id'
[options="header",cols="1,3"]
|=================
| Expression  | Description
| "<string>"  | The specified profile id such as "udp1", "http"
| "_"         | Underscore means default profile. If profile_id is not specified, it is used as the default between JSON-RPC client and server.
| "*"         | Asterisk means all profiles
|=================
...
* *Parameters*
** *handler* ['string'] - unique connection handler
** *profile_id* ['string'] [*optional*] - xref:astf_profile_id['profile id'] associated with this profile
...
|=================
| Field           | Type    | Description 
| frag_first      | boolean | true in case this is the first fragment 
| frag_last       | boolean | true in case this is the last fragment 
| md5             | string  | come in first message with frag_first=true
| fragment        | string  | fragment of input JSON string
|=================
...

{
    "id": "h54xyesm",
    "jsonrpc": "2.0",
    "method": "profile_fragment",
    "params": {
        "api_h": "tJLjtY5D",
        "profile_id": "udp1",
        "frag_first": true,
        "fragment": "{\"buf_list\": [\"\", \"SFRUUC8xLjEgMjAwIE9LDQpTZXJ2ZXI6IE1pY3Jvc29mdC1JSVMvNi4wDQpDb250ZW50LVR5cGU6IHRleHQvaHRtbA0KQ29udGVudC1MZW5ndGg6IDMyMDAwDQoNCjxodG1sPjxwcmU+KioqKioqKioqKjwvcHJlPjwvaHRtbD4=\"], \"ip_gen_dist_list\": [{\"dir\": \"c\", \"distribution\": \"seq\", \"ip_end\": \"16.0.0.255\", \"ip_offset\": \"1.0.0.0\", \"ip_start\": \"16.0.0.0\"}, {\"dir\": \"s\", \"distribution\": \"seq\", \"ip_end\": \"48.0.255.255\", \"ip_offset\": \"1.0.0.0\", \"ip_start\": \"48.0.0.0\"}], \"program_list\": [{\"commands\": [{\"buf_index\": 0, \"name\": \"tx_msg\"}, {\"min_pkts\": 1, \"name\": \"rx_msg\"}], \"stream\": false}, {\"commands\": [{\"min_pkts\": 1, \"name\": \"rx_msg\"}, {\"buf_index\": 1, \"name\": \"tx_msg\"}], \"stream\": false}], \"temp",
        "handler": "vNWxUEPD",
        "md5": "e7a2a3fcbf583a68c25d2d6405d20416"
    }
}
...

{
    ...
    "params": {
        ...
        "profile_id": "udp1",
        "frag_last": true,
        "fragment": "lates\": [{\"client_template\": {\"cluster\": {}, \"cps\": 1, \"ip_gen\": {\"dist_client\": {\"index\": 0}, \"dist_server\": {\"index\": 1}}, \"port\": 80, \"program_index\": 0}, \"server_template\": {\"assoc\": [{\"port\": 80}], \"program_index\": 1}, \"tg_id\": 0}], \"tg_names\": []}",
        "handler": "vNWxUEPD"
    }
}
...
* *API Class* - 'core'
* *Valid States* - 'active'
* *Description* - start traffic on all ports on the loaded profile for a profile id 
* *Parameters*
** *handler* ['string'] - unique connection handler
** *profile_id* ['string'] [*optional*] - xref:astf_profile_id['profile id'] associated with this profile
...
|=================
| Field           | Type    | Description 
| duration        | double  | Stop producing new flows after duration
| mult            | double  | CPS Multiplier
| nc              | bool    | If set, will terminate exactly at the end of the specified duration. This provides faster, more accurate TRex termination. By default (without this option), TRex waits for all flows to terminate gracefully. In case of a very long flow, termination might prolong.
| ipv6            | bool    | convert the profile to ipv6
| latency         | uint32_t| rate of the latency packets. 0 means disable
| client_mask     | uint32_t| mask of client ports. bit is 0 => client is disabled.
|=================
...
* *API Class* - 'core'
* *Valid States* - 'active'
* *Description* - Stop traffic for a profile id if active state 
* *Parameters*
** *handler* ['string'] - unique connection handler
** *profile_id* ['string'] [*optional*] - xref:astf_profile_id['profile id'] associated with this profile
...
|=================
| Field           | Type    | Description 
| mult            | double  | New CPS multiplier (overrides old one)
|=================
...
* *API Class* - 'core'
* *Valid States* - 'active'
* *Description* - clear profile on the loaded state 
* *Parameters*
** *handler* ['string'] - unique connection handler
** *profile_id* ['string'] [*optional*] - xref:astf_profile_id['profile id'] associated with this profile
...
* *API Class* - 'core'
* *Valid States* - 'all'
* *Description* - fetch all the associated profiles
* *Parameters*
** *handler* ['string'] - unique connection handler
...
List of counters descriptions, each have the following metadata:

.Counter description
[options="header",cols="1,1,3"]
|=================
| Field           | Type    | Description 
| id              | uint32_t| counter id
| name            | string  | the name of the counters 
| help            | string  | help string 
| zero            | bool    | show zero counters
| info            | string  | short string for describing 
| real            | bool    | does this counter actually represent some value and not just graphical separator
| abs             | bool    | absolute (non-relative) counters, should not be cleared by clear stats.
|=================
...
=== Get ASTF total counters 

* *Name* - 'get_total_counter_values'
* *API Class* - 'core'
* *Valid States* - 'active'
* *Description* - Get ASTF total counters for all profiles
* *Parameters*
** *handler* ['string'] - unique connection handler
...
'Request':
{
    "id": "10lo4ss3",
    "jsonrpc": "2.0",
    "method": "get_total_counter_values",
    "params": {
        "api_h": "blO17DsZ",
        "profile_id": "_"
    }
...
[
    {
        ...
        "result": {
            "client": {
                ...
                "4": 21148569319.74045,
                "40": 89,
                "5": 1815220.2756911516,
                "6": 1815202.4682634026,
                "62": 2,
                "7": 2056.719394397668,
                "76": 397,
                "77": 378,
            ...
            "server": {
                ...
                "28": 48853281,
                "29": 71147425104,
                "3": 21148557970.405052,
                "30": 229,
                "31": 5,
                "32": 418,
                "4": 7209.544362648545,
                "5": 1815217.5239173174,
                "50": 2,
                "6": 1815204.0503431559,
                "66": 48850418,
                "7": 2004.9566339859036,
                "75": 393,
                "77": 393,
                "78": 12960,
                "79": 393,
...
=== Get Template_Group names
* *Name* - 'get_tg_names'
* *API Class* - 'core'
* *Valid States* - 'all'
* *Description* - get template group names in a profile
* *Parameters* -
** *epoch* ['uint64_t'] - epoch count of Client side for each profile
** *initialized* ['bool'] - init state of Client side for each profile
** *profile_id* ['string'] [*optional*] - xref:astf_profile_id['profile id'] associated with this profile
* *Result* ['object'] - see below
...
=== Get Stats for template group ids
* *Name* - 'get_tg_id_stats'
* *API Class* - 'core'
* *Valid States* - 'all'
* *Description* - get stats for template group Ids in a profile
* *Parameters* -
** *epoch* ['uint64_t'] - epoch count of Client side for each profile
** *profile_id* ['string'] [*optional*] - xref:astf_profile_id['profile id'] associated with this profile
** *tg_ids* ['string'] - array of template group Ids
* *Result* ['object'] - see below
        ...
        "result": {
            "1": {
                "client": {
                    ...
                    "3": 986.3580727717815,
                    "4": 247.8246355847335,
                    "5": 0.49515968561172485,
                    "6": 0.2420162409543991,
                    "7": 209.27547005392645,
                    "76": 1246,
                    "77": 1246,
                    "78": 310254,
                    "79": 1246,
...
=== Start latency streams

* *Name* - 'start_latency'
* *API Class* - 'core'
* *Valid States* - 'active'
* *Description* - start latency ICMP streams in Rx core. used for debug. usually those stream could start using 'start' command using '-l'
* *Parameters*
** *handler* ['string'] - unique connection handler
...
|=================
| Field           | Type    | Description 
| mult            | double  | rate of latency packets
| src_addr        | string  | ipv4 src addr
| dst_addr        | string  | ipv4 dst addr
| dual_port_addr  | string  | dual mask ipv4 addr e.g. 1.0.0.0
| mask            | uint32_t| mask of ports
|=================
...
|=================
| Field           | Type    | Description 
| mult            | double  | new rate of latency packets
|=================
...
se link:cp_docs/api/json_fields.html#trex-latecny-v2-field[stf latency]
...
* *get_global_stats* ['optional'] - make sure the machine is transmitting traffic
* *['perform test']* - perform the required test
...
.get_async_results parameters
[options="header",cols="1,1,3"]
|=================
| Field           | Type    | Description 
| ticket_id       | uint64_t| Ticket ID used to identify the async command being processed.
|=================
...
However, in this case the rx_stats object is enabled and set to monitor ancestor's `stream_id` (which is 0 in this case).
  ...
  stream:
    ...
    vm: [
...

               {
                  ...
                  "name" : "tuple_gen.ip",  # variable to write     
...

               {
                  ...
                  "name" : "tuple_gen.port",  # variable to write     
