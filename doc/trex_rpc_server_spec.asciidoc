...
| total_size      | uint32_t| the size of the fragment 
...
|=================
| Field           | Type    | Description 
| duration        | uint32_t| true in case this is the first fragment 
| mult            | double  | CPS Multiplier 
| nc              | uint32_t| If set, will terminate exacly at the end of the specified duration. This provides faster, more accurate TRex termination. By default (without this option), TRex waits for all flows to terminate gracefully. In case of a very long flow, termination might prolong.
| ipv6            | bool    | convert the profile to ipv6 
| latency         | uint32_t| rate of the latency. 0 means disable
|=================
...
* *API Class* - 'core'
* *Valid States* - 'active'
* *Description* - Update the multiplier 
* *Parameters*
** *handler* ['string'] - unique connection handler
...
each counter have the folowing metadata
...
| real            | bool    | true if double, else uint64
| abs             | bool    | show abs 
|=================
