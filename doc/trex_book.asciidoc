...
[NOTE]
=====================================
 For Intel XXV710 Cisco NIC, make sure to upgrade cimc to latest at lest v4.0(2f), the firmware of the XXV710 will be upgraded to v6 that can support most of sfps.
====================================
...
To check whether a kernel is 64-bit, verify that the output of the following command is `x86_64`.
...
All NICs must have the same type - do not mix different NIC types in one config file. For more info, see link:https://trex-tgn.cisco.com/youtrack/issue/trex-201[trex-201].
...
| --ce | Cores exclude: Black list of cores to exclude. When excluding cores, ensure that enough remain for each NUMA. | --ce 10 11 12
...
----
  - version: 2
    ...
    ...
----
<1> Enable low-end mode.
<2> Set process affinity to this core (default is 0)
...
Socket is for exchange of protocol control messages. (by default /run/vpp/memif.sock in FD.io VPP)
Id just has to match. it's unique for every pair of memifs.
Secret is optional string (by default empty) wich has to match.

==== memif loopback example

To interconnect two memifs into loopback:
...
==== memif interconnected trex and FD.io VPP example

To connect trex with two VPP memif interfaces:
...
VPP CLI commands to create corresponding interfaces:
...
Then the state can be verified by:
...
One can choose the method of how TRex answers to ping/ARP in interactive modes (STL/ASTF).
(Starting from version 2.43)
...
| IPv6:  | [red]*no* | yes
| VLAN:  | yes       | yes
| STL:   | fully     | [red]*in service*
| ASTF:  | fully     | fully
|=================

WARNING: linux_based stack requires service mode to process RX packets in STL mode
...
see link:cp_stl_docs/api/client_code.html#trex.stl.trex_stl_client.STLClient.conf_ipv6[IPv6 configuration]
...
The VLAN Trunk TRex feature attempts to solve the router port bandwidth limitation when the traffic profile is asymmetric (example: Asymmetric EMIX profile).
...
Scenario: TRex with two ports and an EMIX traffic profile.
...
*EMIX traffic without bundling/ALG support*
...
<7> Enable MAC address replacement based on IP. link:trex_manual.html#_static_source_mac_address_setting[Full explanation].
...
<6> If `one_app_server` is set to true, then all flows of this template will use the same server.
<7> If the same server address is required, set this value to true. (Default is false)
<8> New in version v2.62. When set, allows several flows to be present in template. Requires flows_dirs to be configured to specify direction for *each* flow. +
...
<11> New Client IP header offset in bytes. Useful when multiple IP headers are present in the capture (e.g. tunnelled traffic). Overrides default behavior of parsing the first IP header found.
<12> New If IP tunneling is present in the template, it configures the maximum number of tunnels. The outer IPs are adjusted by incrementing them by (inner src IP % max_ip_tunnels).
...
<10> Core to run services on (Scapy, PyBird and Emu). If not given and 'low_end' enables, it will use 'low_end_core' if not given it will use 0. When "low_end" isn't enable it will use 'master_thread_id' and if all wasn't supplied it will use core 0.
...
a configuration file now has the following struct to support multi instance
...
*--software*::
    ...
    Up until *v2.49* (not include) this mode is also limited to using only one TX core (and one RX core as usual). Now it uses multi-queue for RX and TX mainly for scale of virtual interfaces.
...
link:trex_appendix_fixing_pcaps.html[astf-sim for fixing pcaps]

link:trex_appendix_software_mode.html[software mode]
