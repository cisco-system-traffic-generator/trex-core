...
Following TRex version 2.08, "split to core" functionality was deprecated, and remains only for backward compatibility.
The new implementation is always to split as if the profile were sent from one core. It is not necessary for users to consider the actual number of cores.

// Does post v2.08 mean that v2.08 still has "split to core" or does not?
...
----
<1> Deprecated split by field - not in use (post v2.08).

// please clarify connection between "split to core" and "split by field" - not clear how this example relates to the "split to core" issue.


*Some rules regarding split stream variables and burst/multi-burst*::

* When using burst/multi-burst, the packets are split to the default number of threads specified in the YAML configuration file. It is not necessary to split the threads explicitly.
...
*Goal*:: Demonstrate how to assign a stream to a specific core.
...
You need to run TRex with at least two cores for the previous test to work.
...
[source,bash]
----
[bash]>./trex-console -s servername #<1>
----
<1> servername is the name of the server on which you ran TRex.
...
To see the statistics invoke tui by using the following command:
