...
| GUI  | very good | WIP, packet builder, Field Engine, Global port statistic, Latency, per stream statistic . Not the same as IXIA. link:https://github.com/cisco-system-traffic-generator/trex-stateless-gui[trex-stateless-gui]  |
...
*Goal*:: Configure TRex port with either IPv4 or MAC addrees 
...
* When using burst/multi-burst, the number of packets are split to the default number of threads specified in the YAML configuration file, without any need to explicitly split the threads.
...
.Maximum concurrent streams for flow statistics collecting
...
* Gathered info (per stream) is sent using link:http://zguide.zeromq.org/[ZMQ] channel to clients upon request.
...
Shows info about loaded streams on TRex port(s). +
By default, streams are printed as brief summary table:
...
Argument --code can be used either:

* Without argument to print generated Python code that produces the streams.
+
[source,bash]
----
...
* With filename argument to save the streams as Python profile.
+
[source,bash]
----
