...
* Collecting ports. If you don't provide, by default we collect on all ports.
...
We achieve 2 x 20000 x 4000 x 64 = 10 GB. Allocating this amount of memory takes time.

The stats are kept in the Rx thread. The Rx watchdog is disabled prior to starting allocation. The process is async in context of Control Plane to Rx and the waiting for the allocation to finish is done
...
* Unknown Tag, a boolean indicating to get the stats received from unknown tags. Unknown tags are tags that the user didn't provide when enabling TPG.

Let's say the user wants to collect the stats in port 3, for tpgid 1. He wants to collect all the tags from 0-4000. He also wants to know if there were any packets received with a unknown tag.
...
This Api also returns the next tag to start collecting from. In this case it returns 4000 since it collected all the data. But in case there were different stats for each tag, 
we would have received only part of the stats we asked for.
...
[source, bash]
----
trex>tpg_enable --num-tpgids 20 --tags stl/tpg_tags_conf.py -t --min-vlan 1 --max-vlan 20

Enabling Tagged Packet Group                                 [SUCCESS]
----

WARNING: Tunables are supported only in the shown format. Passing tunables in the *-t min-vlan=1,max-vlan=20* format is *not* supported.
...
[source, bash]
----
trex>start -f stl/tpg_1tag_stream.py -t --burst-size 10 --pps 5 --vlans 19 --qinq
----
...
optional arguments:
  ...
  -p PORT, --port PORT  Port that collects stats.
  --tpgid TPGID         Tpgid for which we collect stats
  --min-tag MIN_TAG     Min Tag for which we collect the stats. Defaults to 0.
  --max-tag MAX_TAG     Max Tag for which we collect the stats.
...
[source, bash]
----
trex>tpg_stats -p 0 --tpgid 5 --max-tag 19 --unknown
Port 0, Tpgid 5, Tags = [0, 18]
...
Statistics can be collected by any user for any port that collects TPG stats. For example *user1* can get the statistics of port 2 even if this is not owned by him:
