...
----
    def create_stream (self, direction):
        ...
        vm.var(name="src", min_value=src['start'], max_value=src['end'], size=4, op='inc', 
              split_to_cores = False)                                                       #<1>
        vm.var(name="dst", min_value=dst['start'], max_value=dst['end'], size=4, op='inc')  #<2>
...
In the pcap we can see that each source address is repeated number of cores times, meanwhile the destination addresses are repeated once.
