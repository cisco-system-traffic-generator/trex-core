...
these streams (which can attached to different Data Plane Cores) can put the sequence number out of sync.
...
----
    ...
    try:
        ...
        for i in range(num_streams):
            # stream number = tpgid
            stats = c.get_tpg_stats(port=rx_port, tpgid=i, min_tag=0, 
                                max_tag=num_tags, max_sections=num_tags)[0]  # <5>

            tpgid_passed = verify_stream_stats(stats, streams[i].get_pkt_len(), 
                            rx_port, burst_size, i, verbose, ignore_seq_err) # <6>
            if not tpgid_passed:
                passed = False
...
After the traffic starts, we can collect the stats:
