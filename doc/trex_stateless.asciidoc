...
=== Bird intgration (Beta)

==== Overview
link:https://bird.network.cz/[Bird Internet Routing Daemon] is a project aimed to develop a fully functional dynamic IP routing daemon. It was integrated into TRex to run alongside in order to exploit it's features together with Python automation API. 


==== High level Features 

* BIRD 2.0 Protocols support link:https://bird.network.cz/?get_doc&f=bird.html&v=20[Bird]
** Both IPv4 and IPv6 (using kernel veth)
** BGP (eBGP/iBGP), RPKI (RFC 6480)/RFC 6483
...
* Integration with Multi-RX software model to supprt dynamic filters for BIRD protocols while keeping high rates of traffic 
...
- Run TRex in "ONE_QUEUE" mode and set bird flag on :
...
- In another shell run python script located at: `scripts/automation/trex_control_plane/interactive/trex/examples/stl/add_bird_node.py` (see see link:trex_manual.html#Simple[Simple bird node adding])
...
c.set_service_mode(ports = my_ports, enabled = True)  # this is needed for software mode with one QUEUE, going to be replaced with multi-Rx 
...
cfg = bird_cfg.merge_to_string() # build combined configuration 
c.set_bird_config(config = cfg)  # sending our new config (under the hood there is a PyBird client)

# create 2 veth's in bird namespace, need to move to prom mode to get. You can use the old TRex namespace API this is just a shim to layer t
...
[source,python]
----
bird_config = bird_cfg.merge_to_string()
----
...
3. some commands like: get_config, set_config ...
...
[format="csv",cols="2^,2^,2^", options="header",width="100%"]
|=================
Command name , Query / Command, Description
connect , None, Connect client to PyBird server. Only check versions and open the socket to bird.
acquire,None, Acquire unique "handler" for client. PyBird Server can only acquire 1 client at a time. You may acquire by force: "force=True".
get_config,Query,Return the current bird configuration.
get_protocols_info,Query,Gets the information about the configured protocols as they are appeared in bird. 
set_config,Command,Set the given config string as the new bird configuration. 
set_empty_config,Command,Set emtpy config with no protocols and no routes.
release,None,Release client's handler from server.
disconnect,None,Disconnect client's from server and close the socket. Must be called after releasing client. 
|=================
