...
=== Bird
==== Overview
link:https://bird.network.cz/[Bird Internet Routing Daemon] is a project aimed to develop a fully functional dynamic IP routing daemon. We integrated it into TRex in order to exploit it's features together with TRex and Python automation API.

image::images/bird_logo.png[title="Bird Logo",200, 200, link="images/bird_logo.png"]

==== Topology

Bird process is running in it's own namespace on trex machine. Using veth's to communicate the default namespace and sending data to DUT. Client will use "PyBird Client" in order to send / retrive bird config files (each component described below).
...
We will work with this simple configuration:
