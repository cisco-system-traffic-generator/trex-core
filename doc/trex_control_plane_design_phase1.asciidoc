...
The server version (which runs as either a daemon or a CLI application) is deployed with TRex latest version, written in Python 3.
...
The clients is also based on Python 3, however unlike the server, it can run on any machine who wishes to. +
