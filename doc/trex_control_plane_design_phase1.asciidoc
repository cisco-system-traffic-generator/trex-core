...
These methods are:
  ...
  - **`is_done_warmup()`** +
    ...
    This parameter is important since in most cases, the most relevant test cases are interesting when TRex produces its expected TX, based on which the platform is tested and benchmarked.
