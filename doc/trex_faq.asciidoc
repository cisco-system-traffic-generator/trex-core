...
With advance Stateful mode it done by injecting L7 data on top of TCP/UDP protocols 
...
See this link:trex_stateless.html#_stateful_vs_stateless[in-depth comparison] for more details.
...
You should increase the pool with that raise an error, for example in case of 2048  
