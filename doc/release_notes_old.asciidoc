...
[NOTE]
Newer release notes: link:release_notes.html[link]
...
* ZMQ: enable hearbeat. This is good for situations that the client and server are remote (e.g. The server at the lab and client GUI at DC)  and a firewall between them drop the connection after a long time without keepalive. Another side feature is the ability to set the time-out of the connection for remote connection.
...
* ASTF API documentation is here link:cp_astf_docs/api/client_code.html[python_astf_api] link:trex_astf.html#_tutorial_python_automation_v2_47_and_up[automation]
...
** For backward compatibility old code still works:
+
----
...
** Support all EMIX profile with TCP flows 
