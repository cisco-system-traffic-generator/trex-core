...
=== Change the statistics update frequency
The /opt/napatech3/config/ntservice.ini must be updated to the following:
[source,bash]
----
 [System]
 ...
...
== Change the transmit pointer update interval
The /opt/napatech3/config/ntservice.ini must be updated to the following:
[source,bash]
----
...
...
[source,bash]
----
 [bash]>sed -i -- 's/HostBufferRefreshIntervalTx = default/HostBufferRefreshIntervalTx = 50/g' /opt/napatech3/config/ntservice.ini
 [bash>
----
