...
The TRex EMUProfile includes a number of namespaces. Each namespace is identified by: port, vlan tags(up to 2) and tpids.
Each namespace has a list of clients (empty on initialize). Each client is identified with: mac, ipv4, ipv4 default gateway and ipv6 addresses.
...
There is a difference between a namespace plugin and a client namespace.
