...
def rx_example (tx_port, rx_port, burst_size, pps):
    ...
    try:
        ...
        s1 = STLStream(name = 'rx',
                       packet = pkt,
                       flow_stats = STLFlowLatencyStats(pg_id = 5, multi_tag = True),
        ...
        rc = rx_iteration(c, tx_port, rx_port, total_pkts, pkt.get_pkt_len(), multi_tag)
        if not rc:
            passed = False
...
# RX one iteration
def rx_iteration (c, tx_port, rx_port, total_pkts, pkt_len, multi_tag):
    
    c.clear_stats()
    fetch_vlan_stats = True;
    ...
    for i in range(1,8):
        time.sleep(1)
        stats = c.get_pgid_stats(pgids['latency'], multi_tag)
