...
from console.plugins import ConsolePlugin
import glob
import imp
import argparse
...
class PluginsManager:
    def __init__(self, console):
        ...
        self.stateless_client = console.stateless_client
    ...
    def _unload_plugin(self, names = None):
        try:
            ...
            if not names:
                return
            self.stateless_client.logger.pre_cmd('Unloading plugin%s: %s' % ('s' if len(names) > 1 else '', ', '.join(names)))
            for name in names:
            ...
            self.stateless_client.logger.post_cmd(False)
            self.print_err(e)
            return RC_ERR()
        self.stateless_client.logger.post_cmd(True)
        return RC_OK()
    ...
    def do_plugins(self, line):
        ...
        if opts.plugins_command == 'show':
            ...
            try:
                ...
                self.stateless_client.logger.pre_cmd('Loading plugin: %s' % plugin_name)
                self.stateless_client.logger.post_cmd(False)
                self.print_err(e)
            else:
                self.stateless_client.logger.pre_cmd('Loading plugin: %s' % plugin_name)
            ...
            try:
                ...
                self.stateless_client.logger.log(traceback.format_exc(), 3)
                self.print_err('%s\n' % (e.brief() if isinstance(e, STLError) else e))
