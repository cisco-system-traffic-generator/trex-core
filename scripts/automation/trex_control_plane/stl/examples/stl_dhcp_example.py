...
class DHCPTest(object):
    ...
    def create_dhcp_clients (self, count):
        dhcps = [STLServiceDHCP(mac = random_mac(), verbose_level = STLServiceDHCP.INFO) for _ in range(count)]
    ...
    def ping_from_clients (self, clients):
        ...
        pings = [STLServiceICMP(self.ctx, src_ip = client.get_record().client_ip, dst_ip = client.get_record().server_ip,
                                verbose_level = STLServiceICMP.INFO) \
                 for client in clients]
