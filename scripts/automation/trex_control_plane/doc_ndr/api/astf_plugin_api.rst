Non Drop Rate Plugin API for ASTF
=================================

The following snippet shows a simple yet general implementation of the API. From the documentation we know that in case TRex runs into some error in a transmission it counts that transmission as invalid, and starts transmitting
at a lower multiplier. However, in case some specific errors happen, there is no point into continuing. We can implement this using the following plugin. Also, we can use the plugin for the purpose of logging the errors.
...
.. autoclass:: trex.examples.astf.ndr_plugin.MyNDRPlugin
    :members: 
    :inherited-members:
