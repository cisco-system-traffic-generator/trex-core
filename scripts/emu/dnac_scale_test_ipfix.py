...
class Prof1:
    ...
    def get_profile(self, tuneables):
        ...
        parser.add_argument('--dst-ipv4', type=str, default = '127.0.0.1', dest = 'dst_ipv4', help='Ipv4 address of collector')
        parser.add_argument('--dst-port', type=str, default = '18088', dest = 'dst_port', help='Destination port')
        ...
        parser.add_argument("--devices-num", type=int, default=4000, dest="devices_num", help="Number of devices to generate")
        parser.add_argument("--rampup-time", type=str, default="1s", dest="rampup_time", help="The time duration over which the devices should be generated")
        parser.add_argument("--total-rate-pps", type=int, default=25000, dest="total_rate_pps", help="The total rate (pps) in which all devices export IPFIX packets")
        parser.add_argument("--client-ipv4", type=str, default="1.1.1.1", dest="client_ipv4", help="The total rate (pps) in which all devices export IPFIX packets")
        parser.add_argument("--clients-per-device", type=int, default=48, dest="clients_per_device", help="The total rate (pps) in which all devices export IPFIX packets")
        parser.add_argument("--apps-per-client", type=int, default=41, dest="apps_per_client", help="The total rate (pps) in which all devices export IPFIX packets")
        parser.add_argument("--exporter-use-emu-client-ip-addr", type=bool, default=None, dest="use_emu_client_ip_addr", help="UDP exporter use EMU client IP addr")
