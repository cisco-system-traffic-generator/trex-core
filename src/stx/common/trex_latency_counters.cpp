...
void RXLatency::handle_flow_latency_stats(const rte_mbuf_t *m, uint32_t& ip_id) {
...
void RXLatency::handle_pkt(const rte_mbuf_t *m, int port) {
    ...
    /* In hardware filter mode, only supported IP packets could be handled */
    if (!m_rcv_all) {
        if ((ret != 0) /* unsupported */ || (ret2 != 0) /* non-IP */)
            return;

        if (is_flow_stat_id(ip_id)) {
            if (is_flow_stat_payload_id(ip_id)) {
                handle_flow_latency_stats(m, ip_id);
            }
            else {
    ...
    /* In software mode, non-IP packet could be handled */
    else {
        handle_flow_latency_stats(m, ip_id);
