    ...
    if (CGlobalInfo::is_learn_mode())
    {
        ...
        if (CGlobalInfo::is_learn_mode(CParserOption::LEARN_MODE_TCP) && (pkt_0_indication.getIpProto() == IPPROTO_TCP))
        {
            ...
            if (CGlobalInfo::is_learn_mode(CParserOption::LEARN_MODE_TCP_ACK) && is_tcp)
            {
                ...
                {
                    fprintf(stderr, "Error: Wrong third packet. In the chosen learn mode, need at least the 3 TCP handshake packets.\n");
                    fprintf(stderr, "       Please give different CAP file, or try different --learn-mode\n");
                    return kTCPLearnModeBadFlow;
                }
                if ((pkt_0_indication.m_cap_ipg < (double)LEARN_MODE_MIN_IPG / 1000) || (pkt_1_indication.m_cap_ipg < (double)LEARN_MODE_MIN_IPG / 1000))
                {
                    ...
                    fprintf(stderr, "       Current delay is %f between second and first, %f between third and second", pkt_0_indication.m_cap_ipg, pkt_1_indication.m_cap_ipg);
                    fprintf(stderr, "       Please give different CAP file, try different --learn-mode, or edit ipg parameters in template file\n");
...
bool CNodeGenerator::Create(CFlowGenListPerThread *parent)
{
    ...
    if (CGlobalInfo::m_options.m_is_sleepy_scheduler)
    { // measure nanosleep overhead
...
void CFlowGenListPerThread::init_from_global()
{
    ...
    for (i = 0; i < (int)m_flow_list->m_cap_gen.size(); i++)
    {
        ...
        yaml_info->m_restart_time = (yaml_info->m_limit_was_set) ? (yaml_info->m_limit / (yaml_info->m_k_cps * 1000.0)) : 0;
...
HOT_FUNC inline int CNodeGenerator::flush_file_realtime(dsec_t max_time,
                                                        dsec_t d_time,
                                                        CFlowGenListPerThread *thread,
                                                        double &old_offset)
{
...
bool CNodeGenerator::handle_slow_messages(uint8_t type,
                                          CGenNode *node,
                                          CFlowGenListPerThread *thread,
                                          bool on_terminate)
{
...
void CFlowGenListPerThread::handle_nat_msg(CGenNodeNatInfo *msg)
{
    ...
    for (i = 0; i < msg->m_cnt; i++)
    {
        ...
        {
#ifdef NAT_TRACE_
            printf(" %.03f  RX :set node %p:%x  %x:%x TCP diff %x\n", now_sec(), node, nat_msg->m_fid, nat_msg->m_external_ip, nat_msg->m_external_port, node->get_nat_tcp_seq_diff_client());
#endif
